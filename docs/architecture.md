...
| D5 | 身為被指派教師,我會收到站內通知與 Email,可確認或婉拒(婉拒需填原因) | MVP |
| D6 | 身為教學組長,我能直接指定代課(不經教師確認),適用急件 | MVP |
...
| `teacher` | 教師 | 姓名、任教科目(多)、基本鐘點、行政職稱與減課數、是否外聘/業界師資、在職狀態 |
...
| `notification` | 通知 | 站內+Email;類型、收件人、已讀狀態 |
...
stateDiagram-v2
    ...
    已登記 --> 待處理: 系統展開受影響節次
    待處理 --> 邀請中: 指派代課(需確認)
    待處理 --> 已確認: 直接指定/併班/自習/調課
    邀請中 --> 已確認: 教師接受
    邀請中 --> 待處理: 教師婉拒(附原因)
    邀請中 --> 已確認: 逾時未回→組長改直接指定
    已確認 --> 已完成: 上課日結束自動轉換
    已完成 --> [*]: 計入月鐘點統計
    待處理 --> 已取消: 銷假
    邀請中 --> 已取消: 銷假(通知已邀教師)
...
- 通知管道:站內通知 + Email(v2 加 webhook)。教師端所有操作(查課表/請假/確認代課)必須手機瀏覽器可完成。
