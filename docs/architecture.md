...
| A0 | 身為評估這套系統的人,我能在全新系統上一鍵載入示範資料(18 班國中、48 位教師、384 筆配課),不必先手 key 幾百筆就能試跑自動排課。規格見 `app/data/demo_school.json`,只在「尚無任何學期」時可執行 | v1.2 |
| B3-1 | 身為管理員,我能設定「超鐘點上限」(預設 8 節),配課超出「應授 + 上限」時系統擋下並指名是誰。上限是相對值而非固定節數,因為應授本身因身分而異(臺南市:國文專任 16、兼任導師 11、兼任主任 6)。**未填基本鐘點的教師不受管制**——`base_periods=0` 代表資料未建立,不是真的不用上課,硬擋會讓未維護基鐘的學校連一般配課都做不了 | v1.2 |
...
| S4 | 教師空堂集中(減少零碎空堂) | 低 |
| S5 | 主科(`subject.is_major`)優先排上午 | 中 |
| S6 | 教師連續授課 ≤3 節 | 中 |
...
- **進度與心跳**:進度存於獨立的 Redis hash(`solve:{job_id}`),不放 RQ job meta——RQ 只知道「執行中/失敗」,說不出「已找到 12 個解、目前目標值 148」。worker 每 2 秒送出心跳;API 讀到 `running` 但心跳超過 30 秒未更新即判定 worker 已死,回報明確錯誤而非讓前端永遠轉圈。
- **提前結束 vs 取消**:`stop` 停止搜尋但**保留當下最佳解**並寫出結果草稿;`cancel` 停止並丟棄。兩者都經由 `CpSolver.stop_search()`,由背景執行緒觸發。
...
- **衝突定位是本系統的差異化重點**:市售系統只回「排不出來」,本系統要說出**是哪幾件事湊在一起、鬆開哪一個就好了**,並附上具體數字。
...
改採**刪除法**:把每個旋鈕整組關掉、重新建一個乾淨的模型求解——每次求解都完整 presolve,上例整套定位約 2~3 秒。附帶的好處是**每一條結論都被一次真實的求解驗證過**:報告說「放寬音樂師1 的不可排時段就排得出來」,是因為真的排出來了。

- 旋鈕 = 教學組長改得動的東西:`H4` 某位教師的不可排時段、`H3` 某間場地的互斥、`H10` 全校的每日科目上限、`H9` 鎖定的格位。
- `H1`(班級同時段一門課)與 `H2`(教師同時段一門課)**不是旋鈕**——沒有東西可轉,而且真正的成因(某位教師配課超過可排格數)pre-flight 已經算得出來。
- **報告的誠實邊界**:`each` 模式的每一項都被一次真實求解驗證過(關掉它就排出來了),
  這個結論恆為真;但**清單是否完整**、以及 `joint` 的組合是否最小,取決於時間是否用完、
  以及每次試解是否在時限內判定得出來。任何一次試解回 `unknown`(沒能證明可行、也沒能證明無解)
  都會把 `complete` 降為 `False`,`structural` 的措辭也隨之收斂——
  **絕不宣稱一件從未被證明的事**。
- 報告分三種:
  - `each`:每一項各自都是瓶頸,**放寬任何一項即可**(最常見)。
  - `joint`:必須同時處理多項(先累加到可行,再逐一剔除不必要的)。
  - `structural`:旋鈕全轉到底仍然無解,問題出在配課總量,列出最吃緊的班級與教師。
- **逾時且零解時也要跑定位**:帶著軟約束目標函數的 CP-SAT 常常證不出 INFEASIBLE(見上表)。定位以純硬約束求解,能分辨「不可能」與「只是慢」——這兩件事的處置完全不同。
...
- 懲罰量級刻意拉開:**未排入(10000) ≫ 違反被放寬的約束(1000) ≫ 軟約束(1~8)**。
  ...
  (一節課最多參與約 8 個懲罰項 → 軟約束總代價 < 800 < 1000)。若允許把某項軟約束設到
  20000,solver 會理性地「丟掉一節課」去換取分散度——那是災難。勾了「可放寬教師不可排時段」,意思就是「寧可讓老師委屈一節,也不要讓這門課排不進去」。
- 因此部分排課的 `objective` 與一般模式**尺度不同**(被未排入的懲罰灌爆),UI 顯示「未排 N 節」而不是目標值。
- pre-flight 的把關也隨之放寬:部分排課只擋**結構性**錯誤(連堂比整段連續節次還長、跑班群組節數不一致、某類型場地一間都沒有)——那些連模型都建不起來;「教師配課超量」「場地不夠」正是部分排課要處理的事。
- 結果一律再交給 `validator` 檢查:除了被明確放寬的那類約束以外,其餘硬約束必須一格都沒違反。**「少排幾節」不可以偷偷變成「排錯幾節」。**
