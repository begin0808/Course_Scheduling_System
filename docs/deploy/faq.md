...
你**從 v1.0 升級時沿用了舊的 `docker-compose.yml`**(`worker-ops` 是 v1.1 才新增的容器)。此時匯出、備份、還原、寄信都不會有人處理,而且**每日自動備份也停了**(這一項是無聲的,不會有人來告訴你)。

更新 compose 檔後重啟即可,資料不受影響:

```bash
# 方式 A:重新下載 v1.1 的 docker-compose.yml,方式 B:git pull
docker compose up -d
docker compose ps          # 應看到 worker 與 worker-ops 兩個都在跑
```
