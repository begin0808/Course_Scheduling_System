...
- **代課通知單列印**(照使用學校紙本格式):指派代課後可印出**教師代課通知單**與**班級代課通知單**(A4 直式週課表)。表頭寫代課教師(或代課班級)、請假教師、請假起訖、假別與計費方式;格子裡教師單寫「日期/科目/班級[代]」,班級單寫「日期/科目/代課老師[代]」。**併班**一併印出,標記為「[併]」。
  - 印一筆(該節「列印代課單」)、印一張假單(卡片右上角)、印一段期間(調代課紀錄批次)三種入口,與調課單相同。
  - 同一位代課老師代同一張假單的多節會合併成一張;跨週一週一頁。
  - 新 API:`GET /api/substitute-slips?semester_id=&affected_period_ids=`。
- **代課的「計費方式」**:指派代課時可選公費代課/自費代課/課務自理/不支鐘點,也可自行輸入;印在代課通知單上。預設依假別帶入(事假自費,其餘公費),沿用既有的 `funding_source` 欄位,**沒有資料表結構變更**。
  - 新 API:`GET /api/substitution-funding-sources`。

### 內部
- 調課單與代課單的週課表版面抽成共用模組 `slip_layout`(節次表、下午粗線、跨週分頁);通知單 API 回傳的格子欄位 `teacher_name` 更名為 `actor`(第三行文字,兩種單子寫的東西不同)。

### 文件
- 操作手冊 8.2 補上代課通知單與計費方式的說明與截圖(`13-substitute-slips.png`)。
