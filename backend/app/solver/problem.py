...
# 軟約束權重上限。這不是美觀限制,是**部分排課的正確性前提**:
# 一節課最多參與 S1~S8 約 8 個懲罰項,故其軟約束總代價 < 8 × MAX_WEIGHT = 800,
# 必須小於「違反被放寬的硬約束」(1000),後者又必須小於「整節不排入」(10000)。
# 若權重可設到 20000,solver 會理性地「丟掉一節課」來換取分散度——那是災難。
# 見 model_builder.Relaxation。
MAX_WEIGHT = 100
